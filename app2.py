...
# We'll use SIMILARITY >= SIM_THRESHOLD to accept, else "Unknown".
...
def init_insightface():
    ...
    # ctx_id selects first GPU if CUDA provider exists; set -1 for CPU
    ctx_id = 0 if ("CUDAExecutionProvider" in providers) else -1
    face_app.prepare(ctx_id=ctx_id, det_size=(640, 640))
    # Optional: tighten detector threshold slightly to reduce false-positives
    try:
...
class Worker(threading.Thread):
    ...
    def _detect_and_recognize(self, frame):
        """InsightFace-first pipeline; YOLO is only a fallback for *detection* if ever needed."""
        public_dets = []
        vis = frame.copy()

        # 1) Detect + embed with InsightFace in one pass
        persons = []
        try:
        ...
        # Optional YOLO fallback if InsightFace returns nothing
        if not persons and yolo_model is not None:
            try:
                ...
                for b in boxes:
                    ...
                    # Re-run InsightFace on the crop to get embedding
                    try:
                        sub = face_app.get(face_crop)
                        if sub:
                            obj = sub[0]
                            # Map bbox back to full-frame coords for drawing
...
if YOLO is not None:
    try:
        # if you have face weights locally; else, comment this out and rely on InsightFace only
